...
if [[ -n "${DEEPSEEK_API_KEY:-}" ]]; then
  ...
  if [[ -n "$CONTENT" ]]; then
    echo "  ✓ DeepSeek integrate succeeded with $DEEPSEEK_MODEL" >&2
    echo "$CONTENT"
    exit 0
  fi

  ERR=$(echo "$RESP" | jq -r '.error.message // .error // "no content returned"')
  echo "  ⚠ DeepSeek integrate failed ($DEEPSEEK_MODEL): $ERR, falling back to OpenRouter..." >&2
else
  echo "  → DEEPSEEK_API_KEY not set, using OpenRouter" >&2
fi
